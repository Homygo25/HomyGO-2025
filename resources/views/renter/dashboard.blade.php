...
<body class="bg-gray-50 text-gray-900 min-h-screen">
      ...
      <div class="relative">
        ...
        <div id="dropdownMenu" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg py-2 z-50">
          @auth
            ...
            <a href="{{ route('properties.index') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Browse Properties</a>
            <a href="{{ route('renter.bookings') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">My Bookings</a>
  ...
  <div class="px-4 -mt-8 relative z-10">
    <form action="{{ route('properties.index') }}" method="GET" class="bg-white rounded-lg search-shadow p-2 max-w-md mx-auto">
      <div class="flex items-center">
        <input 
          type="text" 
          name="search"
  ...
  <section class="px-4 py-8">
      ...
      @if(isset($featuredProperties) && $featuredProperties->count() > 0)
        @foreach($featuredProperties as $property)
          ...
          <div class="bg-white rounded-lg card-shadow overflow-hidden">
            <img 
              src="{{ $property->image ? asset('storage/'.$property->image) : 'https://images.unsplash.com/photo-1586023492125-27b2c045efd7?ixlib=rb-4.0.3&auto=format&fit=crop&w=1000&q=80' }}" 
              alt="{{ $property->title }}" 
            ...
            <div class="p-4">
              ...
              <p class="text-gray-600 text-sm mb-3">
                {{ Str::limit($property->description, 50) }} • {{ $property->location }}
              </p>
              <div class="flex items-center justify-between">
                <span class="text-xl font-bold text-gray-900">₱{{ number_format($property->price_per_night) }}</span>
                <span class="text-gray-500 text-sm">/night</span>
              </div>
              <a href="{{ route('properties.show', $property->id) }}" class="block w-full mt-4 bg-green-600 hover:bg-green-700 text-white py-3 rounded-lg font-medium transition-colors duration-200 text-center">
                View Details
              </a>
            </div>
          </div>
      ...
      @else
        ...
        <div class="bg-white rounded-lg card-shadow overflow-hidden">
          ...
          <div class="p-4">
            ...
            <a href="{{ route('properties.index') }}" class="block w-full mt-4 bg-green-600 hover:bg-green-700 text-white py-3 rounded-lg font-medium transition-colors duration-200 text-center">
              View Details
            </a>
          </div>
        </div>
    ...
    <div class="text-center mt-8">
      <a href="{{ route('properties.index') }}" class="inline-block bg-white text-green-600 border-2 border-green-600 hover:bg-green-600 hover:text-white px-8 py-3 rounded-lg font-medium transition-colors duration-200">
        View All Properties
      </a>
    </div>
  </section>
